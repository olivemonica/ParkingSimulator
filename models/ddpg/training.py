        ...
        for epoch in range(nb_epochs):
            ...
            for cycle in range(nb_epoch_cycles):
                ...
                for t_rollout in range(nb_rollout_steps):
                    ...
                    if env.special_ep:
                        ...
                        if env.car.speed >= upper_lim:
                            action[0] = np.clip(np.minimum(action[0],
                                                -0.5 - np.random.exponential(0.3)), -1, 1)
                            #print('slowing down')
                        if env.car.speed <= lower_lim:
                            action[0] = np.clip(np.maximum(action[0],
                                                0.5 + np.random.exponential(0.3)), -1, 1)
                            #print('speeding up')
                        # If car is in spot, stop hard
                        if env.in_spot:
                            action[0] = -1
                            # action[0] = np.clip(np.minimum(action[0],
                            #                     -0.7 - np.random.exponential(0.3)), -1, 1)
                        #print(action)
                        rnd_act = action[0:1]
                    # Execute next action.
                    ...
                    rnd_act = np.clip(np.random.normal(size=len(action), scale=0.3) + max_action * action, -1, 1)
                    #print(action, rnd_act)
                    new_obs, r, done, info = env._step(rnd_act)
                    step_times.append(time.time() - step_start)

                    for i in range(len(info['substep_times'])):
                        substep_times[i].append(info['substep_times'][i])
